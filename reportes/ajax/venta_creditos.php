...
$date_format = explode("-",$_GET['fin']);
	echo '<h1>Reporte: Ventas a creditos al '.$date_format[2].' de '.nombreMes($date_format[1]).' a&ntilde;o '.$date_format[0].'</h1>';
}
else{
	echo '<h1>Reporte General: Ventas a creditos </h1>';	
}
...
?>
<table border="1" cellpadding="0" cellspacing="0"  id="table" width="700">
<tr>
...
//revisamos si la consulta tiene fecha	
	if($_GET['inicio']!='' and $_GET['fin']!=''){
		...
		FROM registro 
		WHERE ID_CLIENTE = '".$row['ID_CLIENTE']."' and fecha BETWEEN '".$_GET['inicio']."' AND DATE_ADD('".$_GET['fin']."', INTERVAL 1 DAY)
		GROUP BY Tipo_comb
		");
	}
	else{
...
*/
	$client= mysql_fetch_array(mysql_query("SELECT ID_CLIENTE,nombre,RNC,telefono FROM clientes WHERE tipo <> 2 AND ID_CLIENTE = '".$row['ID_CLIENTE']."' ORDER BY nombre"));
